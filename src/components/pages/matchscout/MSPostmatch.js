  ...
  return (
    <>
      <Grid2 container spacing={3}>
        {/* Can this be subjectively measured? */}
        <CustomRating
          value={data.get(MatchStage.POST_MATCH, "driver_rating")}
          onChange={(newValue) => {
            data.set(MatchStage.POST_MATCH, "driver_rating", newValue);
            update();
          }}
          title={"Cooperation rating"}
          description={"Did this team help or hinder their alliance"}
        />
        <CustomInput
          required={false}
          label={"Extra Comments"}
          helperText={
            "Anything else you would like to add? For example, did the robot break down, or did the drive team do something exceptional?"
          }
          type={"text"}
          multiline={true}
        ...
        <Collapse in={data.get(MatchStage.POST_MATCH, "defense")}>
          {/* Similarly, can this be subjectively measured? */}

          <CustomRating
            value={data.get(MatchStage.POST_MATCH, "defense_rating")}
            onChange={(newValue) => {
            ...
            description={
              "How well did the driver play defense on their target team?"
            }
          />

          {/* They might not remember this also idt strategy chief would use this 
          info; if they defended 254 well or something super impressive 
          like that then they can add that in the comments */}
          <CustomInput
            label={"Team Defended"}
          ...
          <CustomInput
            ...
            helperText={
              "Did the driver perform so well, that their performance can't even be described by a number?"
            }
            type={"text"}
            multiline={true}
            fullWidth={true}
